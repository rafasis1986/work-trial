...
from facades.worktrialfacade import find_aborted_and_pending_ssr, filter_prid_abortables


def test_get_samples_id():
    q.init_worktrial_scripts()
    response = q.get_sample_ids()
    find_aborted_and_pending_ssr()
    assert len(response) == 9


def test_get_aborted_ssr_list():
    response = q.get_rows_by_table_name('aborted_ssr')
    assert len(response) == 8


def test_get_pending_ssr_list():
    response = q.get_rows_by_table_name('pending_ssr')
